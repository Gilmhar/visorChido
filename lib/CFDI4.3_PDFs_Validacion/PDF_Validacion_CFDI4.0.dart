...
import 'package:visor_xml/utils/Validar_XML.dart';
import 'package:visor_xml/utils/datos_xml.dart';
import 'package:visor_xml/utils/post.dart';

PDFCFDI4Validacion(context, String data) async {

  //Aqui se crea todo el formato del PDF y se le dan los valores necesarios optenidos del XML
  ...
  final validacionXML = validarxmlres();

  List<int> bytes = [];

  //Para crear el PDF primero tenemos que declarar el Documento
  PdfDocument document = PdfDocument();
  //Luego nuestra pagina que es la principal y va en blanco
  final page = document.pages.add();
  //Y declaramos nuestras celdas
  PdfGrid gridG = PdfGrid();
  //Y columnas
  gridG.columns.add(count: 8);


  //Aqui ya agregamos nuestras celdas como rows y la cantidad de columnas
  PdfGridRow pv = gridG.rows.add();
  pv.cells[0].columnSpan = 8;
  //Y ya podemos empezar a agregar los valores, estilo y pocision
  pv.cells[0].value = 'Fecha de Validación: $fecha'; //valor como string
  pv.cells[0].style = pdfValidacionStyle(sinB, 8); // estilo de texto y celda
  pv.cells[0].stringFormat = pdfStrAlin(PdfTextAlignment.right); //pocision
  ...
  pv.cells[6].value = 'Tipo: ${tipoComprobante(dato.tipoDeComprobante)}';
  ...
  if (estado == 'Vigente') {
    ...
    pv.cells[0].value =
        'Fecha de Expedición:  ${dato.fecha.toString().replaceAll('.000', '')}';
    pv.cells[3].columnSpan = 3;
    pv.cells[3].value =
        'Fecha Timbrado:  ${dato.fechaTimbradoT.toString().replaceAll('.000', '')}  ';
    pv.cells[6].columnSpan = 2;
  ...
  pv = gridG.rows.add();
  pv.cells[0].columnSpan = 2;
  pv.cells[0].value = 'Metodo de pago: ${metodoPago(dato.metodoPago)}';
  pv.cells[2].columnSpan = 3;
  ...
  pv = gridG.rows.add();
  pv.cells[0].columnSpan = 8;
  pv.cells[0].value = dato.regimenFiscalReceptor == ''
      ? ''
